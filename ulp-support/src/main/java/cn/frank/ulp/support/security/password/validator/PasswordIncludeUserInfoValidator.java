   ...
   public void validate(String password) throws PasswordInvalidException {
      if (this.enabled) {
         ...
         if (StringUtils.isNoneBlank(this.fullName)) {
            if (this.matchBehavior.match(password, Pinyin4jUtils.getFirstSpellPinYin(this.fullName, true))) {
               logger.error("密码不能包含用户姓名拼音");
               throw new PasswordIncludeUserInfoInvalidException("密码不能包含用户姓名拼音");
            }

            if (this.matchBehavior.match(password, Pinyin4jUtils.getFirstSpellPinYin(this.fullName, false))
                || this.matchBehavior.match(password, Pinyin4jUtils.getPinYinHeadChar(this.fullName))) {
               logger.error("密码不能包含用户姓名拼音");
               throw new PasswordIncludeUserInfoInvalidException("密码不能包含用户姓名拼音");
            }
         }
         ...
         if (StringUtils.isNoneBlank(this.nickName)) {
            ...
            if (this.matchBehavior.match(password, Pinyin4jUtils.getFirstSpellPinYin(this.nickName, true))) {
               logger.error("密码不能包含昵称拼音");
               throw new PasswordIncludeUserInfoInvalidException("密码不能包含昵称拼音");
            }

            if (this.matchBehavior.match(password, Pinyin4jUtils.getFirstSpellPinYin(this.nickName, false))
                || this.matchBehavior.match(password, Pinyin4jUtils.getPinYinHeadChar(this.nickName))) {
               logger.error("密码不能包含昵称拼音");
               throw new PasswordIncludeUserInfoInvalidException("密码不能包含昵称拼音");
            }
         }
