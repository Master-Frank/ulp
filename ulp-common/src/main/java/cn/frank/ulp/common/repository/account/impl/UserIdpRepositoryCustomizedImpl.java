...
import cn.frank.ulp.common.repository.account.UserIdpRepositoryCustomized;
...
public class UserIdpRepositoryCustomizedImpl implements UserIdpRepositoryCustomized {
    ...
    public Optional<UserIdpBindPO> selectById(String id) {
        String hql = """
                SELECT
                    NEW cn.frank.ulp.common.entity.account.po.UserIdpBindPO(uib.id,user.id,user.username,user.nickName,user.email,user.phone,user.phoneAreaCode,user.avatar,tpu.openId,tpu.unionId,idp.id,idp.name,idp.type,uib.bindTime)
                FROM
    ...
    public Iterable<UserIdpBindPO> getUserIdpBindList(String userId) {
        String hql = """
                SELECT
                    NEW cn.frank.ulp.common.entity.account.po.UserIdpBindPO(uib.id,user.id,user.username,user.nickName,user.email,user.phone,user.phoneAreaCode,user.avatar,uib.thirdPartyUser.openId,uib.thirdPartyUser.unionId,idp.id,idp.name,idp.type,uib.bindTime)
                FROM
                    UserIdpBindEntity uib
                    LEFT JOIN UserEntity user ON uib.userId = user.id
